...
@Repository
@Validated
public class OrderRepositoryImpl extends AbstractReactiveRepository implements OrderRepository {

    // SQL Queries
    private static final String SQL_SELECT_ORDER_BY_ID =
            "SELECT id, status, correlation_id FROM orders WHERE id = :id";
    private static final String SQL_SELECT_ORDER_STATUS_BY_ID =
    ...
    public Mono<OrderStatus> getOrderStatus(@NotNull Long orderId) {
        ...
        return databaseClient.sql(SQL_SELECT_ORDER_STATUS_BY_ID)
                .bind("id", orderId)
                .map((row, metadata) -> {
                    try {
                        String statusString = row.get("status", String.class);
                        return OrderStatus.fromValue(statusString);
                    } catch (Exception e) {
                        String errorMsg = "Error mapping order status: " + e.getMessage();
                ...
                .transform(mono -> withTimeoutsAndRetries(mono, "getOrderStatus"))
                .onErrorResume(TimeoutException.class, e -> {
                    String errorMsg = "Database timeout getting order status: " + e.getMessage();
                    log.error(errorMsg, e);
                ...
                .onErrorResume(e -> {
                    ...
                    return handleDatabaseError(e, "getOrderStatus");
                });
    }
